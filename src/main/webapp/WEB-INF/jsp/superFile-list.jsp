	...
	<body>
		...
		$(document).ready(function() {
			$("#dataGridTable").jqGrid({
				url: "${ctx}/manage/superFile/query",
				datatype: "json",
				colModel: [
			...
			$("#btn-add").click(function() {
				var url = "${ctx}/manage/superFile/add?random=" + Math.random();
				var options = {
					title: '新增',
					width: 500
				};
		...
		*/
		...
		function doModify(id) {
			var url = "${ctx}/manage/superFile/edit?id=" + id + "&random=" + Math.random();
			var options = {
				title: '编辑',
				width: 500
			};
		...
		function doTrash(id) {
			dialog({
			    ...
			    ok: function () {
			        this.title('删除中…');
			        var url = "${ctx}/manage/superFile/delete?random="+ Math.random();
					var params = {
						id: id
					};
