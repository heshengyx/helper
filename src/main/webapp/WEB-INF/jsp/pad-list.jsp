...
<html lang="en">
	...
	<label class="col-sm-1 control-label no-padding-right" for="nameQuery">名称</label>
	<div class="col-sm-11">
<input type="text" id="nameQuery" placeholder="名称" class="col-xs-10 col-sm-5" />
</div>
</div>
<div class="form-group">
	<label class="col-sm-1 control-label no-padding-right" for="titleQuery">标题</label>
	<div class="col-sm-11">
<input type="text" id="titleQuery" placeholder="标题" class="col-xs-10 col-sm-5" />
</div>
</div>
							...
							<div class="form-group">
								<div class="col-md-offset-1 col-md-11">
									...
									&nbsp; &nbsp; &nbsp;
									<button class="btn btn-sm btn-info" type="button" id="btn-export"><i class="icon-plus bigger-110"></i>导 出</button>
								</div>
							</div>
						</form>
...
{label:'标题', name:'title', index:'title'},
			...
			$('#btn-search').click(function() {
				var params = "";
				params += "&name=" + $("#nameQuery").val();
				params += "&title=" + $("#titleQuery").val();
				$('#params').val(params);
				$('#dataGridTable').jqGrid('setGridParam', {page:1});
				doSearch();
			});
			$("#btn-add").click(function() {
				var url = "${ctx}/manage/pad/add?random=" + Math.random();
				var options = {
					title: '新增',
					width: 500
				};
				...
				if (params) {
					dialog({
						...
						ok: function () {
							...
							var url = '${ctx}/manage/pad/export?random=' + Math.random() + params;
		...
		*/
		...
		function doSearch() {
			...
			$("#dataGridTable").jqGrid("setGridParam", {
				url : "${ctx}/manage/pad/query?random="+ Math.random(),
				page : page,
				postData : {
					name: $("#nameQuery").val(),
title: $("#titleQuery").val(),
		...
		function doModify(id) {
			var url = "${ctx}/manage/pad/edit?id=" + id + "&random=" + Math.random();
			var options = {
				title: '编辑',
				width: 500
			};
		...
		function doTrash(id) {
			dialog({
			    ...
			    ok: function () {
			        this.title('删除中…');
			        var url = "${ctx}/manage/pad/delete?random="+ Math.random();
					var params = {
						id: id
					};
